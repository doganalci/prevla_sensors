...
//  source devel/setup.bash &&   roslaunch ti_mmwave_rospkg 1843_multi_3d_0.launch
#include <iostream>
using std::cerr;
using std::endl;
#include <fstream>
...
DataUARTHandler::DataUARTHandler(ros::NodeHandle* nh) : currentBufp(&pingPongBuffers[0]) , nextBufp(&pingPongBuffers[1]) {
    ...
    ROS_INFO("\n\n==============================\nList of parameters\n==============================\nNumber of range samples: %d\nNumber of chirps: %d\nf_s: %.3f MHz\nf_c: %.3f GHz\nBandwidth: %.3f MHz\nPRI: %.3f us\nFrame time: %.3f ms\nMax range: %.3f m\nRange resolution: %.3f m\nMax Doppler: +-%.3f m/s\nDoppler resolution: %.3f m/s\n==============================\n", \
        ...
        if (cnf==1)    bb=sprintf (buffer_cfg, "\"Numof range samp: %d, Numof chirps: %d, f_s: %.3f MHz, f_c: %.3f GHz, Bandwidth: %.3f MHz, PRI: %.3f us, Frame time: %.3f ms, Mx range: %.3f m, Range res: %.3f m Mx Dopl: +-%.3f m/s, Doppler res : %.3f m/s\"",nr, nd, fs/1e6, fc/1e9, BW/1e6, PRI*1e6, tfr*1e3, max_range, vrange, max_vel/2, vvel);
...
void *DataUARTHandler::sortIncomingData( void )
{
      ...
      if (sy==0)
      {
        sy++;
        std::string line;
        //if (false){ifstream fradar2("/home/doga18/Desktop/mmwave_ti_ros/ros_driver/log/radar_data_doga.cvs");          while (getline(fradar2, line))son_radar++;              }
        //if (false){ifstream fimage2("/home/doga18/Desktop/mmwave_ti_ros/ros_driver/log/image_point_cvs.cvs");       std::string line2;   while (getline(fimage2, line2))son_image++; }
        ifstream fdeney2("/home/doga18/Desktop/mmwave_ti_ros/ros_driver/log/deney_loglari.cvs");      std::string line3;    while (getline(fdeney2, line3))son_deney++;
        //  printf("->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n\n\n%d\n",son_radar );printf("%d\n",son_deney );printf("%d\n",son_image );
      }

  //                      std::ofstream outdata( "/home/doga18/Desktop/mmwave_ti_ros/ros_driver/log/radar_data_doga.dat", std::ios::app ) ;
//                        std::ofstream cvsdata( "/home/doga18/Desktop/mmwave_ti_ros/ros_driver/log/radar_data_doga.cvs", std::ios::app ) ;


//
                        //outdata.open("/home/doga18/Desktop/mmwave_ti_ros/ros_driver/log/radar_data_doga.dat",std::ios_base::app); // opens the file
                        cvsdata.open("/home/doga18/Desktop/mmwave_ti_ros/ros_driver/log/radar_data_doga.csv",std::ios_base::app); // opens the file
  ...
  //  fractional_seconds_since_epoch=std::chrono::duration_cast<std::chrono::duration<double>>(        std::chrono::system_clock::now().time_since_epoch()).count();
        ...
        if (terminal==1 && t==0){
          ...
          char c1[250] ="gnome-terminal -- bash -c ' python3 /home/doga18/Desktop/mmwave_ti_ros/ros_driver/src/ti_mmwave_rospkg/src/hand_media.py ";

          char cam_p[10 + sizeof(char)];
          std::sprintf(cam_p, "%d", cam_prt);

          char cb[10]=" ";          char ct[10]="\"";
          strcat(c1, cam_p);
          strcat(c1,cb);
          char c3[250] = " 0.0000000001 1 1 a 1 1 ";
          char c4[250] = " ; exec bash'";
          char sc[10]="";
          char num_char[10 + sizeof(char)];std::sprintf(num_char, "%d", scale_factor);
          char son_dny[10 + sizeof(char)];std::sprintf(son_dny, "%d", son_deney);
          char im_op[10 + sizeof(char)];std::sprintf(im_op   , "%d", image_operation);
          strcat(c1, c3);
          strcat(c1,kimlik);
          strcat(c1,cb);
          strcat(c1,num_char);
          ...
          strcat(c1,cb);
          strcat(c1,buffer_cfg);
          strcat(c1,cb);
          strcat(c1,c4);
          int exit_status1 =system("gnome-terminal -- bash -c 'tail -f  /home/doga18/Desktop/mmwave_ti_ros/ros_driver/log/radar_data_doga.csv; exec bash'");
          //int exit_status2 =system("gnome-terminal -- bash -c 'tail -f  /home/doga18/Desktop/mmwave_ti_ros/ros_driver/log/radar_data_doga.dat; exec bash'");
          int exit_status3 =system("gnome-terminal -- bash -c 'tail -f  /home/doga18/Desktop/mmwave_ti_ros/ros_driver/log/image_point_cvs.cvs; exec bash'");
          //int exit_status4 =system("gnome-terminal -- bash -c 'tail -f  /home/doga18/Desktop/mmwave_ti_ros/ros_driver/log/image_connect_cvs.cvs; exec bash'");
          int exit_status5 =system("gnome-terminal -- bash -c 'tail -f  /home/doga18/Desktop/mmwave_ti_ros/ros_driver/log/deney_loglari.cvs; exec bash'");
        ...
        //int exit_status3 =system("gnome-terminal -- bash -c '  python3 /home/doga18/Desktop/mmwave_ti_ros/ros_driver/hand/hand_media.py 0 0.0000000001 1 1 a 1 1 doga; exec bash'");
...
char ttt[250]="";
                    ...
                    printf("\n  hey yooo \n" );
                                        ...
                                        if (bag==0){
                                          ...
                                          //  rosbag::Bag bag("/home/doga18/Desktop/mmwave_ti_ros/ros_driver/src/ti_mmwave_rospkg/log/test.bag");
/*                                            std_msgs::Int32 ii;
                                            ii.data = 42;
                                            bag.write("numbers", ros::Time::now(), ii);
                                            bag.close();
